var sript_origin = document.scripts[document.scripts.length - 1].src.replace(
		"angular-beans.js", "");
...
var angularBeans = {
	bind : function(scope, service, modelsName) {
		...
		scope
				.$watch(angular.toJson(modelsName).split('"').join(""),
						function(newValue, oldValue) {
							for (i in modelsName)
								scope[modelsName[i].split(service.serviceID
										+ ".")[1]] = newValue[i]
						}, true)
	},
